...
const handleFilter = async() => { 
  ...
  if(formattedFromDate == "" || formattedToDate == ""){
   toast.error("fill or change both from and to date!");
    ...
    try {
      const response = await fetch(`${baseUrl}/mgtransactionfilter`, requestOptions);
...
const handleMonthlyFilter = async(e: React.ChangeEvent<HTMLInputElement> | null) => { 
 ...
 const fetchForMonth = async(month : number, year :  number) => {     
    ...
    try {
      const response = await fetch(`${baseUrl}/mgtransactionfiltermonth`, requestOptions);
      if (!response.ok) {
        const errorResponse = await response.json();
        throw new Error(errorResponse.message);
      }
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
              const response = await fetch(`${baseUrl}/mgtransaction/${userId}`, requestOptions);
              const result = await response.json(); 
              setProducts(result.data); 
              setLoading(false);
            } catch (error) {
