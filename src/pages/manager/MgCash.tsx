...
const handleFilter = async() => { 
  ...
  if(formattedFromDate == "" || formattedToDate == ""){
   toast.error("fill or change both from and to date!");
    ...
    try {
      const response = await fetch(`${baseUrl}/mgpurchasecashfilter`, requestOptions);
...
const handleMonthlyFilter = async(e: React.ChangeEvent<HTMLInputElement> | null) => { 
 ...
 const fetchForMonth = async(month : number, year :  number) => {     
    ...
    try {
      const response = await fetch(`${baseUrl}/mgpurchasecashfilterbymonth`, requestOptions);
      if (!response.ok) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
              const response = await fetch(`${baseUrl}/mgpurchasecash/${userId}`, requestOptions);
              if (!response.ok) {
                const errorResponse = await response.json();
                throw new Error(errorResponse.message);
              }
