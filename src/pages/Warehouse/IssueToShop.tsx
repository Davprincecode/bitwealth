...
function IssueToShop() {
    ...
    const [destinationName, setDestinationName] = useState<string>('new market shop');
    const [destinationType, setDestinationType] = useState<string>('shop');
  ...
  const handleSelectChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const handleIssuedProduct = async (type : string) => {
        setLoading(true);
        const myHeaders = new Headers();
        myHeaders.append("Content-Type", "application/json");
         myHeaders.append("Authorization", token);
        ...
        try {
          const response = await fetch(`${baseUrl}/issuetoshop`, requestOptions); 
    ...
    <div className="btn">
        {
                  destinationName && data.length > 0 ? (
                    <button onClick={() => handleIssuedProduct("shop")} disabled={loading}>
