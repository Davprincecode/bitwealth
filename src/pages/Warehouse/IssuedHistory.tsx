...
function PurchaseHistory() {
  ...
  const handleFilter = async() => { 
    if(formattedFromDate == "" || formattedToDate == ""){
     toast.error("fill or change both from and to date!");
      ...
      try {
        const response = await fetch(`${baseUrl}/issuedgoodsfilter`, requestOptions);
        ...
        const filteredPrd = result.data.filter((prd: { status: string; }) => prd.status === 'paid');
  ...
  const handleMonthlyFilter = async(e: React.ChangeEvent<HTMLInputElement> | null) => { 
   ...
   const fetchForMonth = async(month : number, year :  number) => {     
    ...
    setLoading(true);
      ...
      try {
        const response = await fetch(`${baseUrl}/issuedgoodsmonth`, requestOptions);
        if (!response.ok) {
          const errorResponse = await response.json();
          throw new Error(errorResponse.message);
        }
    ...
    useEffect(() => {
        const fetchData = async () => {          
            ...
            try {
              const response = await fetch(`${baseUrl}/issuedgoods`, requestOptions);
              const result = await response.json(); 
              setProducts(result.data); 
              const filteredPrd = result.data.filter((prd: { status: string; }) => prd.status === 'paid');
              setFilteredProducts(filteredPrd);
  ...
  const switchStatus = (status : string) => {
    setStatus(status);
    if(status === "paid"){
      const filteredPrd = products.filter((product) => product.status === 'paid');
      setFilteredProducts(filteredPrd);
    }
    if(status === "part payment"){
        const filteredPrd = products.filter((product) => product.status === 'part payment');
        setFilteredProducts(filteredPrd);
    }
    if(status === "credit"){
        const filteredPrd = products.filter((product) => product.status === 'credit');
        setFilteredProducts(filteredPrd);
    }
  } 
...
{/* ================================ */}
{
    ...
    }} onClick={()=>switchStatus("paid")}>
                <p>paid</p>
            </div>
            <div className="part payment"  style={{ 
    ...(status === "part payment" && { 
      color: "#EB232D", 
      borderBottom: "2px solid", 
      marginBottom: "-1px" 
    }) 
  }} onClick={()=>switchStatus("part payment")}>
                <p>part payment</p>
            </div>
...
<table className="styled-table">
    ...
    <tr  key={id}>
        ...
        <td>
            ...
            <NavLink to={`/showIssuedHistory/${user.issuedId}`}>
              <FaEye />
            </NavLink>
            </div>
            </div>
...
status !== "part payment" && status !== "credit" && (
<table className="styled-table">
<thead>
<tr>
            <th>No</th>
